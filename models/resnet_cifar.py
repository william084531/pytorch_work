...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            residual = self.downsample(x)#這對去除則可拿掉resnet
...
def resnet20_cifar(**kwargs):
    model = ResNet_Cifar(BasicBlock, [3, 3, 3], **kwargs)# 6*3 +2 矩陣內的數字乘以6(6層) +2(在取矩陣) 
    return model


def resnet32_cifar(**kwargs):
    model = ResNet_Cifar(BasicBlock, [5, 5, 5], **kwargs)# 6*5 +2
    return model
